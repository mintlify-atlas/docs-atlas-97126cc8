...
Four cards sit at the top-left of the grid, each built with a Recharts `AreaChart` sparkline that plots the last 10 data points. The cards and their delta calculations are:

<CardGroup cols={2}>
  <Card title="Consultas hoy" icon="calendar">
  ...
  <Card title="Pacientes totales" icon="users">
  ...
  <Card title="Ingresos del mes" icon="trending-up">
    Sum of `price` for all appointments with status `completada` in the current calendar month. The delta compares this figure against the same-day-of-month total from the previous month (e.g., on the 15th it compares month-to-date with the first 15 days of last month).
  </Card>
  <Card title="Actividad ahora" icon="activity">
    A live activity panel — not a numeric KPI — that shows three progress bars: room occupancy (scaled from today's appointment count), completed appointment percentage, and a static 94 % satisfaction indicator.
  </Card>
</CardGroup>
...
- **Total this month**: sum of `price` for `completada` and `programada` appointments in the current calendar month, shown in a sage-coloured badge.
