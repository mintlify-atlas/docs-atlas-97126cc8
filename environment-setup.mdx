---
title: "Configure Supabase and Environment Variables"
sidebarTitle: "Environment Setup"
description: "Set up your Supabase project, apply the database schema, configure required environment variables, and prepare FemeSalud for local development or production."
---
...
<Warning>
  Never commit your `.env` file to version control. Add it to `.gitignore` immediately. Although `VITE_SUPABASE_ANON_KEY` is a **public** key designed for browser use, leaking your full `.env` file can still expose server-only secrets you add later (such as a service role key or third-party API credentials).
</Warning>
...
| `VITE_SUPABASE_ANON_KEY` | Client + Server | Supabase `anon` / public key. Found in **Project Settings → API → Project API Keys**. |
...
| `clinic_info` | Clinic-level configuration (name, address, logo, contact details) |
